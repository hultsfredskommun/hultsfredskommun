...
function readMoreToggle(el){
	...
	if( !$(article).hasClass("loaded") ){
		...
		$(morediv).load(hultsfred_object["templateDir"]+"/ajax/single_post_load.php",{id:post_id,blog_id:hultsfred_object["blogId"]}, function()
		{
			...
			// contact popup
			$(this).find(".contact-area a.contactlink").each(function() {
...
$(document).ready(function(){
	...
	$(".dropdown-menu").unbind("click").click( function(){
		...
		if( $("ul.main-menu").is(":visible") ){
			$("ul.main-menu").slideUp('fast');
			$("ul.main-sub-menu").slideUp('fast');
		}
		else{ 
			$("ul.main-menu").slideDown('fast'); 
			$("ul.main-sub-menu").slideDown('fast'); 
		}
	
	});  
	$(".dropdown-nav").unbind("click").click( function(){
