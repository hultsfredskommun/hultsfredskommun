		...
		if (typeof google != 'undefined') {
			$(this).each(function() {
				...
				$(this).height(height).gmap({scrollwheel: false, center: coordinates, zoom: 15, callback: function() {
					var self = this;
					self.addMarker({'position': this.get('map').getCenter()}).on("click touchstart",function() {
		...
		if (typeof google != 'undefined') {
			$(this).each(function() {
				...
				$(this).on("click touchstart", function(ev) {
					...
					$(".close-contact").on("click touchstart",function() {
						$(".contact-popup").remove();
					});

					$(".contact-popup.overlay").on("click touchstart",function() {
						$(".contact-popup").remove();
					});
...
function readMoreToggle(el){
		...
		// show full content
			...
			$(article).find('.summary-content').slideUp(200, function(){
				...
				$(this).parents("article").find('.more-content').slideDown(200, function(){
					...
					var closea = $('<div>').addClass('closeButton button top').html("<i class='i' data-icon='&#xF148;'></i><a href='#'>Visa mindre</a>").on("click touchstart",function(ev){
						ev.preventDefault();
						readMoreToggle( $(this).parents("article").find(".entry-title a") );
					});
					var closeb = $('<div>').addClass('closeButton button bottom').html("<i class='i' data-icon='&#xF148;'></i><a href='#'>Visa mindre</a>").on("click touchstart",function(ev){
						ev.preventDefault();
						readMoreToggle( $(this).parents("article").find(".entry-title a") );
					});
	...
	if( !$(article).hasClass("loaded") ){
		...
		$(morediv).load(hultsfred_object["templateDir"]+"/ajax/single_post_load.php",{id:post_id,blog_id:hultsfred_object["blogId"]}, function()
		{
			...
			$(print_link).on("click touchstart",function(ev){
...
$(document).ready(function(){
	...
	$(".js-view-summary").on("click touchstart",function(ev){
	...
	$(".js-view-titles").on("click touchstart",function(ev){
	...
	$('#scrollTo_top a').on("click touchstart",function(){
	 ...
	 */
	$(".js-show-main-menu").on("click touchstart",function(ev) {
		$(".main-menu").toggleClass("unhidden");
		ev.preventDefault();
	});
	$(".js-show-main-sub-menu").on("click touchstart",function(ev) {
	...
	/*$(".js-show-category-menu").on("click touchstart", function(){
		$(".category-navigation").toggleClass("unhidden");
		ev.preventDefault();
	});*/  
...
function setArticleActions(el) {
	...
	/* add print dialog */
	$("article .side-content").find(".tool-line.print").on("click touchstart",function(ev) {
	...
	$(el).find(".js-read-click").on("click touchstart",function(ev) {
		ev.preventDefault();
		$(this).parent().next().find(".readspeaker_toolbox").fadeToggle();
	});
	/* add AddThis onclick */
	$(el).find(".js-friend-click").on("click touchstart",function(ev) {
		ev.preventDefault();
	...
	$(el).find('.entry-title a, .togglearticle, .openButton').on("click touchstart",function(ev){
		ev.stopPropagation();
		ev.preventDefault();
	...
	$(el).find(".summary-content").on("click touchstart",function(){
		readMoreToggle( $(this).parents("article").find('.entry-title a') );
	});
		...
		$('#dyn-posts-placeholder').hide().load(hultsfred_object["templateDir"]+"/ajax/posts_load.php",
			...
			function() {
				...
				if (true) {
					...
					$('#dyn-posts-load-posts a').on("click touchstart",function(ev) {
...
function responsive_menu() {
	...
	$(".js-more-menu-click").on("click touchstart",function(ev) {
