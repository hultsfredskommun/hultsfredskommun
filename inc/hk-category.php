		...
		if ($_REQUEST["orderby"] == "") :
			...
			if ( $cat != "" || $tag != "") :
				...
				if ( !empty($sticky) ) {
					...
					if ($tag == "") {
						$args["post_type"] = array('post','hk_kontakter');
					}
					else {
						$args["post_type"] = array('post','attachment','hk_kontakter');
					}
				...
				if ($tag != "") : // show from children when tag is selected
					...
					// only get posts if category has children
					if (!empty($args['category__in']) {
						...
						if (!empty($sticky)) {
							if ($tag == "") {
								...
								$args["post_type"] = array('post','hk_kontakter','attachment');
							}
							$args['post_status'] = 'publish';		
